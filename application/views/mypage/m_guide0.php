		<div class="profileWrap1">
			<div class="title0">
				<div class="underline">关于微导</div>
			</div>  <!-- 라인 들어가는 타이틀 city부분 복제 -->
			
			<img src="/application/views/images/mypage/img_guide1.png" alt="guide_image" class="mb40">
			<div class="guideimgBand">
				<span class="guide_txt">马上成为微导游</span>
				<span class="guide_next" data-toggle="modal" data-target="#guideModal">
			</div>
			<div class="guideimgBand3">
					<p  data-toggle="modal" data-target="#guideModal">>> 了解更多关于成为导游</p>
			</div>
			



			<div id="guidehomeWrap">
				<div class="guidetxtWrap">
					<div class="cols1">
						<img src="/application/views/images/mypage/col1.png" alt="guide_image2">
						<p class="colsP1">海峡时报</p>
						<p>马来西亚农业与农基工业部长依斯迈沙比利1日在社交媒体发文号召马来人抵制华人商家，以强迫后者降价。依斯迈沙比利称，“大多数消费者是马来人，华人只是少数，如果马来人一起抵制华人的生意，他们肯定别无选择，只能降价马来西亚农业与农基工业部长依斯迈沙比利1日在社交媒体发文号召马来人抵制华人商家，以强迫后者降价。依斯迈沙比利称，“大多数消费者是马来人，华人只是少数，如果马来人一起抵制华人的生意，他们肯定别无选择，只能降价马来西亚农业与农基工业部长依斯迈沙比利1日在社交媒体发文号召马来人抵制华人商家，以强迫后者降价。依斯迈沙比利称，“大多数消费者是马来人，华人只是少数，如果马来人一起抵制华人的生意，他们肯定别无选择，只能降价</p>
						<a href="#">更多</a>
					</div>
					<div class="cols2">
						<img src="/application/views/images/mypage/col2.png" alt="guide_image2">
						<p class="colsP2">东亚日报</p>
						<p>据法国民意研究机构IFOP于19日公布的民意调查结果显示，巴黎恐怖袭击事件发生后，法国总统奥朗德的支持率直线上升，目前已达40%，而此前他的支持率仅为19%。报道称，调查结果充分证明，政治危机也可以是领导人的机会。法国民意研究机构IFOP于19日公布的民意调查结果显示，巴黎恐怖袭击事件发生后，法国总统奥朗德的支持率直线上升，目前已达40%，而此前他的支持率仅为19%。报道称，调查结果充分证明，政治危机也可以是领导人的机会。 </p>
						<a href="#">更多</a>
					</div>
					<div class="cols3">
						<img src="/application/views/images/mypage/col3.png" alt="guide_image2">
						<p class="colsP3">产经新闻</p>
						<p>据美国疾病防治中心（CDC）表示，美国目前的麻疹病例急速增加。去年年末发生在加利福尼亚州迪士尼乐园的疫情正持续蔓延，截止当地时间2日，美国14个州已经确诊102例病例。为此，美国疾病防治中心也呼吁人们接种疫苗。据美国疾病防治中心（CDC）表示，美国目前的麻疹病例急速增加。去年年末发生在加利福尼亚州迪士尼乐园的疫情正持续蔓延，截止当地时间2日，美国14个州已经确诊102例病例。</p>
						<a href="#">更多</a>
					</div>
				</div><!-- guidetxtWrap end -->
			</div> <!-- guidehomeWrap -->

		</div><!-- profileWrap1 -->


		<!-- modal 시작 -->
		<div class="modal fade" id="guideModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
				<div id="guidemodal-dialog" class="modal-dialog">
					<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
						<h4 class="modal-title txt_blue" id="exampleModalLabel">了解更多关于成为导游</h4>
					</div>
					<div class="modal-body">
						<p>
							感谢您成为Journey On!卓尼的微导。有您的加入， 微旅行文化将得到更多的支持。<br><br>

							在这里，您可以分享您熟识的城市里的风光和活动。<br>
							让准备出发的旅客，在他们的行程里加入地道体验，留下最深刻的旅行回忆。<br><br>

							不需要是专业导游，您也可以安排和定制小小的旅行计划， 当他们的朋友，带旅客去看最地道的东西，告诉他们最地道的知识。您会从中得到回报，不论是以金钱或是一顿饭的形式，还有回忆和友谊都是最珍贵的。<br><br>
							在您上传微旅程之前，请仔细阅读以下事项:<br><br><br><br>
							<ul class="nav nav-tabs">
							  <li role="presentation" class="active"><a href="#">上传/编辑微旅程</a></li>
							  <li role="presentation"><a href="#">微导认证</a></li>
							  <li role="presentation"><a href="#">微旅程咨询记录</a></li>
							  <li role="presentation"><a href="#">其他事项</a></li>
							</ul>
								我们鼓励微导们善用他们的时间，在空闲的日子里腾出时间做导游。当然如果旅客和您本人同意，您也可以成为他们整个旅程的旅伴。<br>
微旅行的要点是独特性。 不需要长时间， 不要求高消费，只要作为当地人的您都认同这个地方/活动，想必旅客是会更有兴趣的。<br>
编辑微旅程时，请务必填写所有资料，加入您认为最具代表性的照片（最多5张）并写上标题；<br>
定制合适的价格，并标记价格所包括和不包括的事项。特别的细节可以在该微旅程的Q&A或是私信内和旅客交代商议。<br>
上传后如需要编辑，可以在微旅程管理的页面里操作。<br><br><br><br>
							<ul class="nav nav-tabs">
							  <li role="presentation"><a href="#">上传/编辑微旅程</a></li>
							  <li role="presentation" class="active"><a href="#">微导认证</a></li>
							  <li role="presentation"><a href="#">微旅程咨询记录</a></li>
							  <li role="presentation"><a href="#">其他事项</a></li>
							</ul>
只要您在个人页面中填写所有项目，然后在微导认证申请页面中按‘申请认证’，经过审查后，您将会得到微导认证。<br>
在微导认证申请的页面里，可以查看申请进度。当我们需要您提供更多个人资料，在流程标示中会显示不足的原因，您从新上传即可继续申请。<br>
申请专业微导认证时，您需要上传您的专业导游证书。经过审查后，您将会得到专业微导认证。<br>
当您得到微导认证/专业微导认证，旅客在选择和预订微旅程时会更安心和肯定。<br><br><br><br>
							<ul class="nav nav-tabs">
							  <li role="presentation"><a href="#">上传/编辑微旅程</a></li>
							  <li role="presentation"><a href="#">微导认证</a></li>
							  <li role="presentation" class="active"><a href="#">微旅程咨询记录</a></li>
							  <li role="presentation"><a href="#">其他事项</a></li>
							</ul>
旅客们可以从您上传的微旅程里向您提出咨询，进入主页右上方的信息图示，您可以查看您和其他卓尼用户的信息交流记录。<br>
如果收到可疑或骚扰消息，请您马上向我们举报。<br>
注意：切勿告知任何人您的密码/银行账户或其他私隐资料。<br><br><br><br>
							<ul class="nav nav-tabs">
							  <li role="presentation"><a href="#">上传/编辑微旅程</a></li>
							  <li role="presentation"><a href="#">微导认证</a></li>
							  <li role="presentation"><a href="#">微旅程咨询记录</a></li>
							  <li role="presentation" class="active"><a href="#">其他事项</a></li>
							</ul>
没有登入的访客不能看到。。。。。。。。。。<br>
如果您在上传微旅程时，发现任何技术问题，随是欢迎您跟我们联络。<br><br><br><br>
						</p>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
					</div>
				</div>
			</div>
		</div>
		<!-- modal 끝 -->