<div id="detailCity_wrap">
	<section id="left_section">
		<!-- <div class="4menu">
			<button class="btn btn-danger">微旅行</button>
			<button class="btn btn-danger" onclick="pageQna()">疑问解答</button>
			<button class="btn btn-danger">介绍</button>
		</div> -->

		<div class="contents_wrap">
			<div class="title"><div class="underline">微旅行</div></div>  <!-- 라인 들어가는 타이틀 city부분 복제 -->
			<div class="contents_text1">游轮上的演员第一次看到自由女神的激动和自豪，就像你身临其境一样。自由女神像表达了美国对新移民的欢迎</div>	
			<div class="contents_img">
				<div class="img_big"><img src="/application/views/images/contents/img_big.jpg" alt="img_big"></div>
				<div class="img1"><img src="/application/views/images/contents/img_a.jpg" alt="img_a"></div>
				<div class="img2"><img src="/application/views/images/contents/img_b.jpg" alt="img_b"></div>
				<div class="img3"><img src="/application/views/images/contents/img_c.jpg" alt="img_c"></div>
				<div class="img4"><img src="/application/views/images/contents/img_d.jpg" alt="img_d"></div>
			</div>
			<div class="contents_text2"><p>有着“自由照耀世界之神”美誉的美国自由女神像，是美国自由的象征，她拥有迷人的容颜和气宇轩昂的气质，高高屹立在自由岛上，让每一个刚刚抵达纽约港的人，都能远远的看见她。当轮船抵达纽约港的时候，就可以远远地望见美丽的自由女神，你可以感受到经典电影《海上钢琴师》里面的场景，当游轮上的演员第一次看到自由女神的激动和自豪，就像你身临其境一样。自由女神像表达了美国对新移民的欢迎，也成为美国和自由的标志。你可以选择游艇的方式，近距离</p>
			<p>有着“自由照耀世界之神”美誉的美国自由女神像，是美国自由的象征，她拥有迷人的容颜和气宇轩昂的气质，高高屹立在自由岛上，让每一个刚刚抵达纽约港的人，都能远远的看见她。当轮船抵达纽约港的时候，就可以远远地望见美丽的自由女神，你可以感受到经典电影《海上钢琴师》里面的场景，当游轮上的演员第一次看到自由女神的激动和自豪，就像你身临其境一样。自由女神像表达了美国对新移民的欢迎，也成为美国和自由的标志。你可以选择游艇的方式，近距离</p>
			<p>有着“自由照耀世界之神”美誉的美国自由女神像，是美国自由的象征，她拥有迷人的容颜和气宇轩昂的气质，高高屹立在自由岛上，让每一个刚刚抵达纽约港的人，都能远远的看见她。当轮船抵达纽约港的时候，就可以远远地望见美丽的自由女神，你可以感受到经典电影《海上钢琴师》里面的场景，当游轮上的演员第一次看到自由女神的激动和自豪，就像你身临其境一样。自由女神像表达了美国对新移民的欢迎，也成为美国和自由的标志。你可以选择游艇的方式，近距离</p>
			</div>
			<div class="information_wrap">
				<dl class="information">
					<dt><span><img src="/application/views/images/contents/icon30_micro.PNG" alt=""></span>微旅行时间</dt>
					<dd>8小时</dd>

					<dt><span><img src="/application/views/images/contents/icon31_included.PNG" alt=""></span>费用包括</dt>
					<dd>价格不包括</dd>
					<dd>- 餐费</dd>
					<dd>- 景点门票等</dd>
					<dd>- 交通费</dd>
					
					<dt><span><img src="/application/views/images/contents/icon32_notincluded.PNG" alt="">费用不包括</dt>
					<dd>价格不包括</dd>
					<dd>- 餐费</dd>
					<dd>- 景点门票等</dd>
					<dd>- 交通费</dd>

					<dt><span><img src="/application/views/images/contents/icon33_first.PNG" alt="">见面地点</dt>
					<dd>72 Grove Street, New York, NY, United States</dd>

					<dt><span><img src="/application/views/images/contents/icon34_cancel.PNG" alt="">取消规定</dt>
					<dd>24小时内取消</dd>

					<dt><span><img src="/application/views/images/contents/icon35_info.PNG" alt="">其他</dt>
					<dd>etc</dd>
				</dl>
			</div>
			<div class="reply"><!-- 덧글달리는 부분 -->
				<div class="profile_pic">
					<img src="/application/views/images/main/profile01.jpg" alt="" class="img-circle profile_image">
					<img src="/application/views/images/left_menu/flag03_usa.png" alt="" class="flag_image">
					<img src="/application/views/images/main/img22_vmark.PNG" alt="" class="vmark_image">
				</div>
				<div class="reply_text"><span class="reply_id">From Regina Paul : </span>世界的瑰宝呀，逛了一天都没看完，太漂亮了，拍了美美的照片，下次还想来呢回复有用 0 世界的瑰宝呀，逛了一天都没看完，太漂亮了，拍了美美的照片，下次还想来呢回复有用 0世界的瑰宝呀，逛了一天都没看完，太漂亮了，拍了美美的照片，下次还想来呢回复有用 0
				</div>
			</div>

			<div class="reply"><!-- 덧글 답변달리는 부분 -->
				<img src="/application/views/images/contents/reply.png" alt="" class="reply_arrow">
				<div class="profile_pic_answer">
					<img src="/application/views/images/main/profile02.jpg" alt="" class="img-circle profile_image">
					<img src="/application/views/images/left_menu/flag03_usa.png" alt="" class="flag_image">
					<img src="/application/views/images/main/img22_vmark.PNG" alt="" class="vmark_image">
				</div>
				<div class="reply_text_answer"><span class="reply_id">From xiangyu19880319 : </span>世界的瑰宝呀，逛了一天都没看完，太漂亮了，拍了美美的照片，下次还想来呢回复有用 0 世界的瑰宝呀，逛了一天都没看完，太漂亮了，拍了美美的照片，下次还想来呢回复有用 0世界的瑰宝呀，逛了一天都没看完，太漂亮了，拍了美美的照片，下次还想来呢回复有用 0世界的瑰宝呀，逛了一天都没看完，太漂亮了，拍了美美的照片，下次还想来呢回复有用 0 世界的瑰宝呀，逛了一天都没看完，太漂亮了，拍了美美的照片，下次还想来呢回复有用 0世界的瑰宝呀，逛了一天都没看完，太漂亮了，拍了美美的照片，下次还想来呢回复有用 0世界的瑰宝呀，逛了一天都没看完，太漂亮了，
				</div>
			</div>
		</div>
	</section>
	
	<!-- detailcity_aside.php 로 뺀 부분 CI로딩 -->
	<div id='viewContents' name='viewContents'><?$this->load->view("/contents/detailCity_aside");?></div>

</div>


<script>
	function pageQna()
	{
		alert("1");
		//document.getElementById('viewContents').innerHTML='/contents/detailcity_contents.php';

		//location.href = "<?=site_url('City/country/pageQna'); ?>";
	}
</script>